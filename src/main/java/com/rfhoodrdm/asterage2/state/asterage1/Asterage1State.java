	...
	implements RequiresLoadedData
{
	/*	**********************************************************************
		...
		********************************************************************** */
	//global game data.
	@Getter	private int extraLives;					//number of lives remaining for the player.
	@Getter private int level;						//current game level.
	@Getter private int collectedMythiciteTotal;	//how much mythicite collected.
	
	@Getter	private int livesAwarded;				//how many 1-ups has the player earned?
	...
	@Getter @Setter	private GAME_STATUS gameStatus;
	private PlayerShip playerShip;
	private ConcurrentLinkedQueue<SpaceObject> spaceObjectList;
	@Getter @Setter private MessageText officialMessage;		//TODO: Convert this to an optional. Right now we're using null as no-value, which is not safe.
	...
	// TODO: make defensive copy instead?
	...
	public void spawnNewTrollScout ( ConcurrentLinkedQueue<SpaceObject> spaceObjectList )	{
		...
		if ( Math.random() > .5 )	{
			//along top
			double x = Math.random() * GameConstants.GAME_BOARD_WIDTH;
			newScout = new TrollScout ( x, 0 );
		} else {
			//along side.
			double y = Math.random() * GameConstants.GAME_BOARD_HEIGHT;
	...
	public void spawnNewAsteroid ( Asteroid.ASTEROID_SIZE whatSize, ConcurrentLinkedQueue<SpaceObject> spaceObjectList)	{
		...
		double asteroidXRandom = ( Math.random() * GameConstants.GAME_BOARD_WIDTH / 2 );
		double asteroidYRandom = ( Math.random() * GameConstants.GAME_BOARD_HEIGHT / 2 );
		
		//spawn some distance away from the player.
		double asteroidX = (playerX + asteroidXRandom + (GameConstants.GAME_BOARD_WIDTH / 4)) % GameConstants.GAME_BOARD_WIDTH;
		double asteroidY = (playerY + asteroidYRandom + (GameConstants.GAME_BOARD_HEIGHT / 4))% GameConstants.GAME_BOARD_HEIGHT;
